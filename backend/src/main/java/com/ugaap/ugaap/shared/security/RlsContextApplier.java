...
public class RlsContextApplier {
    ...
    public void applyCurrentUserContext() {
        ...
        entityManager.createNativeQuery(
                "SELECT set_config('app.current_branch_id', :branchId, true)"
        ).setParameter("branchId", branchId.map(UUID::toString).orElse("")).getSingleResult();

        entityManager.createNativeQuery(
                "SELECT set_config('app.bypass_rls', :bypass, true)"
        ).setParameter("bypass", String.valueOf(bypass)).getSingleResult();
