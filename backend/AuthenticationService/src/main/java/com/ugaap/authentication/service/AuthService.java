...
public class AuthService {
    ...
    private final Map<String, Long>   memoryStoreTtl = new ConcurrentHashMap<>();

    // ── Store helpers (Redis with in-memory fallback) ──────────────────────

    private void storeValue(String key, String value, long ttlMs) {
        if (redisTemplate != null) {
            try {
                redisTemplate.opsForValue().set(key, value, ttlMs, TimeUnit.MILLISECONDS);
                return;
            } catch (Exception e) {
                log.warn("Redis unavailable, falling back to in-memory store: {}", e.getMessage());
            }
        }
        memoryStore.put(key, value);
        memoryStoreTtl.put(key, System.currentTimeMillis() + ttlMs);
    }
    ...
    private String getValue(String key) {
        ...
        // Check TTL expiry
        Long expiry = memoryStoreTtl.get(key);
        if (expiry != null && System.currentTimeMillis() > expiry) {
            memoryStore.remove(key);
            memoryStoreTtl.remove(key);
            return null;
        }
    ...
    private void deleteValue(String key) {
        if (redisTemplate != null) {
            try {
                ...
                log.warn("Redis unavailable, deleting from in-memory store: {}", e.getMessage());
            }
        }
        memoryStore.remove(key);
        memoryStoreTtl.remove(key);
    ...
    public TempTokenResponse loginStep1(LoginRequest request,
        ...
        storeValue("login_otp:" + tempToken, payload, 5 * 60 * 1000L); // 5 min TTL
    ...
    public LoginResponse loginStep2(OtpVerifyRequest request,
        ...
        String payload  = getValue(redisKey);
    ...
    @Transactional
    public CredentialsResult provisionCredentials(UUID userId, String username,
    ...
    public void deactivateCredentials(UUID userId) {
        ...
        credentialsRepository.save(credentials);
        deleteValue(REDIS_SESSION_PREFIX + userId);
    ...
    public void logout(String accessToken, HttpServletRequest httpRequest) {
        ...
        if (ttl > 0) {
            storeValue(REDIS_BLACKLIST_PREFIX + accessToken, "revoked", ttl);
        }

        deleteValue(REDIS_SESSION_PREFIX + userId);

        sessionRepository.findByRefreshToken(getRefreshTokenForClient(userId))
    ...
    public LoginResponse refresh(RefreshRequest request,
        ...
        String currentValidToken = getValue(redisKey);
    ...
    public boolean isTokenBlacklisted(String token) {
        return hasKey(REDIS_BLACKLIST_PREFIX + token);
    }
    ...
    private LoginResponse generateFullAuthResponse(
        ...
        String accessToken = jwtUtil.generateAccessToken(
                credentials.getUserId(), credentials.getEmail(),
                credentials.getUsername(),
                claims.tenantId() != null ? claims.tenantId() : "",
                claims.branchId() != null ? claims.branchId() : "",
    ...
    private String getRefreshTokenForClient(String userId) {
        String token = getValue(REDIS_SESSION_PREFIX + userId);
