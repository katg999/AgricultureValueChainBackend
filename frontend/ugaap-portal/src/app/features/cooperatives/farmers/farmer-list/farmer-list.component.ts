...
export class FarmerListComponent {
  ...
  // 1. Confirm action with the user
    const confirmed = confirm(`Are you sure you want to approve ${farmer.name}?`);
    if (!confirmed) return;

  // 2. Execute the state change synchronously in your mock service
  const updatedProfile = this.farmerService.approve(farmer.id);

    if (updatedProfile) {
      // 3. Sync the UI table row's state to match the new profile data
      farmer.status = updatedProfile.status; 
      farmer.stage = updatedProfile.stage;
    } else {
      alert('Could not find the target farmer profile to approve.');
    }
}
}