...
export class FarmerDeliveryService {
  private readonly _deliveries$ = new BehaviorSubject<FarmerDelivery[]>([]);
  readonly deliveries$ = this._deliveries$.asObservable();

  private readonly baseUrl = API_ENDPOINTS.BRANCH.FARMER_DELIVERIES; // Points to /api/v1/deliveries

  constructor(private readonly http: HttpClient) {}

  /**
   * GET /api/v1/deliveries/paginated
   * Leverages server-side filtering and pagination schemas matching the Spring controller spec.
   */
  getPaginated(
    page: number, 
    size: number, 
   ...
   * Strict text criteria filter match
   */
   ...
   * Submits a new delivery verification entry to the ledger.
   */
  create(dto: FarmerDeliveryCreateDTO): Observable<FarmerDelivery> {
    return this.http.post<FarmerDelivery>(this.baseUrl, dto).pipe(
      tap(newDelivery => {
        // Appends the raw entity safely directly to your application cache stream
        const currentList = this._deliveries$.value;
        this._deliveries$.next([newDelivery, ...currentList]);
      }),
      catchError(this.handleError)
    );
  }
   ...
   * Modifies an existing delivery record inline.
   */
    ...
    return this.http.patch<FarmerDelivery>(`${this.baseUrl}/${id}`, dto).pipe(
      tap(updatedDelivery => {
        ...
        if (index !== -1) {
          ...
          this._deliveries$.next(updatedList);
        }
      }),
      catchError(this.handleError)
    );
   ...
   * Removes a record from the persistent datastore.
   */
  delete(id: string): Observable<void> {
    return this.http.delete<void>(`${this.baseUrl}/${id}`).pipe(
      tap(() => {
        const currentList = this._deliveries$.value;
        this._deliveries$.next(currentList.filter(d => d.id !== id));
      }),
      catchError(this.handleError)
    );
  }
  ...
  /**
   * Standard error handling channel wrapper
   */
