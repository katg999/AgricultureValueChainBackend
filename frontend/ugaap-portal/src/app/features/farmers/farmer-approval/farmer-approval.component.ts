...
//
// TODO: The farmer ID should come from the route param instead of being
//       hardcoded.  Update farmers.routes.ts to "approval/:id" and inject
//       ActivatedRoute to read it.
// ─────────────────────────────────────────────────────────────────────────────
...
export class FarmerApprovalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // TODO: Replace 'FRM-003' with this.route.snapshot.paramMap.get('id')
    //       once the route is updated to "approval/:id"
    this.farmerService.getById('FRM-003').subscribe({
